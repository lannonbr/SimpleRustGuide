...
\section{Paradigms}
Rust has the following paradigms:
\begin{itemize}
\item Imperative
...
As Rust just hist 1.0 this past spring, it is still a relatively young language. The language is starting to focus on backwards compatibility, but still is defining itself, so changes that may change the syntax may occur. Also, it does not support event-driven programming, which is used mainly in web applications where you interact with an interface. As well, there is no standard way of creating GUIs in Rust.
